...
bot_token = '5484310907:AAHqaQsFv9--D2u3foG3kS_vXhgQjB8KDhE'
    ...
    def get_user_text(message):
        if '@IamKitty_bot ' in message.text:
            if 'мау' in message.text.lower():
                bot.send_message(message.chat.id, 'мау-мау', parse_mode='html')
            elif 'читать' in message.text or 'книж' in message.text.lower():
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton('Почитать о котикаx',
                                                      url='https://ru.wikipedia.org/wiki/%D0%9A%D0%BE%D1%88%D0%BA%D0%B0'))
                bot.send_message(message.chat.id, 'Когда хочется почитать, почитайте о котиках! Вам понравиться, мау',
                                 reply_markup=markup)
                ...
                def answer(message):
                    ...
                    bot.send_message(message.chat.id, f'Держите, мау {url}')
                asking(message)
            ...
            #             message.chat.id, 'Ладно мышеботенькая, я вас не буду кушать, чесьненько!')
                ...
                bot.send_message(message.chat.id, 'У меня всё котошо, спасибо')
                ...
                bot.send_photo(message.chat.id, photo=open('cat.png', 'rb'))
